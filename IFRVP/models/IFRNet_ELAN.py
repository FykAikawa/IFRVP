...
class ResBlock_ELAN(nn.Module):
    def __init__(self, in_channels, bias=True):
        super(ResBlock_ELAN, self).__init__()
        ...
        self.conv7 = nn.Sequential(
            nn.Conv2d(self.in_channels_dv*7, in_channels, kernel_size=1, stride=1, padding=0, bias=bias), 
        )

        #self.prelu = nn.PReLU(in_channels)

    def forward(self, x):
        c1 = x[:, :self.in_channels_dv, :, :]
        c2 = x[:, self.in_channels_dv:self.in_channels_dv*2, :, :]
        c3 = x[:, self.in_channels_dv*2:, :, :]
        c4 = self.conv1(c2)
        c5 = self.conv2(c4)
        c6 = self.conv3(c3)
        c6 = self.conv4(c6)
        c7 = self.conv5(c6)
        c7 = self.conv6(c7)
        out = torch.cat([c1,c2,c3,c4,c5,c6,c7],axis=1)
        out = self.conv7(out) # don't use residual connection
        out = out+x
...
class Model(nn.Module):
    ...
    def forward(self, img0, img1, embt, imgt, flow=None):
        ...
        loss_geo = 0.5 * self.vggloss(imgt_pred,imgt)
